...
const MODALS = [
  MODAL_NAMES.EDIT_STUDENT,
  MODAL_NAMES.DELETE_STUDENT,
  MODAL_NAMES.CREATE_STUDENT,
];
...
const generateInitialState = (stateName?: string): IModalState => {
  if (stateName) {
    return MODALS.filter((m) => m !== stateName).reduce(
...
export const ModalReducer = (
  state: Readonly<IModalState> = initialState,
  action: ModalActions
): IModalState => {
  switch (action.type) {
    case MODAL_ACTION_TYPES.TOGGLE_MODAL:
      return {
        [action.data.stateName]: !state[action.data.stateName],
        ...generateInitialState(action.data.stateName),
      };
